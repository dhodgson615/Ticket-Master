...
class TestDataScraper(unittest.TestCase):
    ...
    def test_scrape_all(self):
        ...
        with patch.object(self.scraper, 'scrape_repository_info') as mock_repo_info, \
             patch.object(self.scraper, 'scrape_git_history') as mock_git, \
             patch.object(self.scraper, 'scrape_file_structure') as mock_files, \
             patch.object(self.scraper, 'scrape_content_analysis') as mock_content, \
             patch.object(self.scraper, 'scrape_dependencies') as mock_deps, \
             patch.object(self.scraper, 'scrape_build_configuration') as mock_build, \
             patch.object(self.scraper, 'analyze_code_quality') as mock_quality, \
             patch.object(self.scraper, 'analyze_activity_patterns') as mock_activity:
    ...
    def test_scrape_git_history(self):
        ...
        with patch.object(self.scraper, '_analyze_commits') as mock_commits, \
             patch.object(self.scraper, '_analyze_contributors') as mock_contributors, \
    ...
    def test_analyze_activity_patterns(self):
        ...
        with patch.object(self.scraper, '_analyze_commit_frequency') as mock_freq, \
             patch.object(self.scraper, '_analyze_time_patterns') as mock_time, \
             patch.object(self.scraper, '_analyze_file_hotspots') as mock_hotspots, \
             patch.object(self.scraper, '_analyze_contributor_activity') as mock_activity:
...
class TestDataScraperCaching(unittest.TestCase):
    ...
    def test_get_from_cache_with_db(self, mock_db):
        ...
        result = scraper._get_from_cache("test_key")
        self.assertTrue(mock_db_instance.method_calls)  # Some method was called
...
class TestDataScraperPrivateMethods(unittest.TestCase):
    ...
    def test_scrape_repository_info_detailed(self, mock_remote, mock_config):
        ...
        mock_remote.return_value = [{"name": "origin", "url": "https://github.com/test/repo.git"}]
    ...
    def test_analyze_commits(self):
        ...
        mock_commits = [
            {"hash": "abc123", "message": "Initial commit", "author": "test@example.com", "date": "2023-01-01"},
            {"hash": "def456", "message": "Add feature", "author": "test@example.com", "date": "2023-01-02"},
        ]
        
        with patch.object(self.scraper.repository, 'get_commits') as mock_get_commits:
    ...
    def test_analyze_contributors(self):
        ...
        self.assertEqual(contributors_by_email["alice@example.com"]["commit_count"], 2)
        self.assertEqual(contributors_by_email["bob@example.com"]["commit_count"], 1)
