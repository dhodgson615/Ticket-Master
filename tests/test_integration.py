...
class TestEndToEndIntegration(unittest.TestCase):
    ...
    def test_llm_issue_generation_pipeline(self):
        ...
        self.assertIsInstance(issues, list)
        self.assertGreater(len(issues), 0)
        self.assertLessEqual(len(issues), self.test_config["issue_generation"]["max_issues"])
        
        # Check issue structure
        for issue in issues:
            self.assertIsNotNone(issue.title)
            self.assertIsNotNone(issue.description)
            self.assertTrue(len(issue.description) >= self.test_config["issue_generation"]["min_description_length"])
