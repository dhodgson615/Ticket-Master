...
class Issue:
    ...
    def create_bulk_issues(
        ...
        logger.info(f"Starting bulk creation of {len(issues)} issues for {repo_name}")
        ...
        for batch_start in range(0, len(issues), batch_size):
            ...
            logger.info(f"Processing batch {batch_start//batch_size + 1}: issues {batch_start+1}-{batch_end}")
            ...
            for i, issue in enumerate(batch):
                try:
                    ...
                    logger.info(f"Successfully created issue {batch_start + i + 1}/{len(issues)}: {issue.title}")
                    ...
                    errors.append(f"Issue {batch_start + i + 1} ({issue.title}): {e}")
                    
                    logger.error(f"Failed to create issue {batch_start + i + 1}/{len(issues)}: {e}")
        ...
        **bulk_options
        ...
        for i, data in enumerate(template_data):
            try:
                ...
                error_msg = f"Failed to create Issue object from template {i+1}: {e}"
        ...
        result = Issue.create_bulk_issues(issues, repo_name, token, **bulk_options)
