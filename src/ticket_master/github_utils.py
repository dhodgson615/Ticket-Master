...
class GitHubUtils:
    ...
    def is_public_repository(self, github_repo: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"GitHub API rate limited, attempting to clone {github_repo} to test public access")
                try:
                    ...
                    self.logger.warning(f"Could not determine repository visibility via clone test: {e}")
                    return False
            else:
                self.logger.warning(f"Unexpected response code {response.status_code} when checking repository visibility")
                return False
    ...
    def get_repository_info(self, github_repo: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"GitHub API rate limited, will attempt direct clone")
                return {
    ...
    def parse_github_url(self, github_input: str) -> str:
        ...
        if '/' in github_input and not github_input.startswith(('http://', 'https://')):
        ...
        if github_input.startswith(('http://', 'https://')):
            ...
            if parsed.netloc.lower() not in ('github.com', 'www.github.com'):
                raise ValueError(f"URL must be from github.com, got: {parsed.netloc}")
            
            path_parts = [part for part in parsed.path.split('/') if part]
            if len(path_parts) >= 2:
                return f"{path_parts[0]}/{path_parts[1]}"
        
        raise ValueError(f"Invalid GitHub repository format: {github_input}. "
                        "Expected formats: 'owner/repo' or 'https://github.com/owner/repo'")
    
    def clone_repository(self, github_repo: str, local_path: Optional[str] = None, 
        ...
        try:
            ...
            if not repo_info:
                raise GitHubCloneError(f"Repository {github_repo} not found or not accessible")
            ...
            if local_path:
                ...
                temp_dir = tempfile.mkdtemp(prefix=f"ticket-master-{github_repo.replace('/', '-')}-")
            ...
            repo = Repo.clone_from(clone_url, target_path, depth=50)  # Shallow clone for performance
            ...
            raise GitHubCloneError(f"Unexpected error cloning {github_repo}: {e}")
